...
\label{verbtemplate}\\
	\lsptoprule
	&{clitic} & \multicolumn{3}{c}{{prefix slots}}	&{stem}	&\multicolumn{5}{c}{{slots}}\\ 
	\cmidrule{3-5}
	\cmidrule{7-11}
	...
	\midrule
	{person}&&{\isit{undergoer}:}\newline \First, \Second, {\Third} or \textsc{middle}&&&&&&&&{actor:}\newline \First, \Second\textbar\Third or $\varnothing$\\ 
	\midrule
	{{gender}}&&{undergoer}: \Tsg.\F, \Tsg.\Masc&&&&&&&&\\
	\midrule
	{number}&&{\isit{undergoer}:} \Sg, \Nsg &{dual}: $\varnothing$- {non-dual}: \emph{a-}&&&&{dual}: \emph{-n} \mbox{non-dual: \emph{-nzr, -wr, -r}}&&&{actor:} \Sg, \Nsg\\
\midrule
	{deixis}& \Prox: \emph{z=}&&&&&&&&\\
	\&& \Med: \emph{b=}	&&&{\isit{ventive}:} \emph{n-}&&&&&{andative:} \emph{-o}&\\
	{direction}& \Dist: \emph{f=}&&&&&&&&&\\
	\midrule
	
	\multirow{2}{*}{\textsc{tam}}&{apprehensive}: \emph{m=}& {\isit{prefix series}:}&{irrealis:}\emph{ra-}&&{stem type:}&{\isit{stative}:}~\emph{-thgr}&&{\isit{past}:} \emph{-a}&&{imperative:}\\
...
% 	{class}&{rule} &\Ext{} &\Rs{} &{gloss} &{count}\\\midrule
	...
	&\Rs{} {only} &- & \emph{-kuk}\footnote{This verb has a second stem \emph{-kogr}, which I analyse as a \isi{positional} stem (see \S\ref{positionalverbs}).} &stand &1 \\
...
\label{argalignverbs} 
	\begin{tabularx}{\textwidth}{Q@{~~~}Qp{15mm}QlQ}
		...
		\midrule 
		prefixing (agent)\footnote{This is a marginal pattern as almost all prefixing verbs have stative semantics.} &\isit{experiencer}, &\Zero{} &n/a &\Abs &intransitive (stative)\\ 
		\tablevspace
		...
		\tablevspace
		\isit{middle} &n/a &\emph{a-} &agent &\Erg{} (\Abs)\footnote{In \isit{suppressed-object} clauses, the \isit{object} is suppressed from the indexation in the verb.}	& suppressed-\isit{object}\\ 
		\tablevspace
...
Prefixing verbs are a small class with around 20 lexical items attested so far. Some of them can occur in other templates, but most occur only in the prefixing template. \tabref{pref.verbs} lists all the members of the prefixing class. Furthermore, there is a class of 41 \isi{positional} verbs, which can occur in the prefixing template (see \S{}\ref{positionalverbs}).
...
The \isi{person} suffix differentiates two \isi{person} values: first and non-first \isi{person}. Thus, second and third \isi{person} are always neutralised and additional information from the personal pronouns or from context is required. As I will explain below, in certain morphological contexts, even this basic distinction is neutralised and only \isi{number} marking is retained. \tabref{perssuff} lists the suffix forms in indicative and irrealis mood.
