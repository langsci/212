...
Words with fixed \isi{gender} allow us to set up some general semantic rules. For example, elongated, big objects are usually \isi{masculine}, while small round objects are \isi{feminine}. Lexemes related to place and land are usually \isi{feminine}. Abstract concepts or nominalised verbs are usually \isi{feminine}. Most fish species are \isi{masculine}, with the exception of the numerous catfish types, which are all \isi{feminine}. Other species, like birds, are much more varied. \tabref{wordclasses-gendersem} gives an overview of the semantic characteristics and lists some examples as well as exceptions.%\\
...
Historically, \emph{nä bun} seems to derive from a combination of \emph{nä} plus the second person singular \isi{dative} \isi{pronoun} \emph{bun} (see \tabref{perspron}), but it is unclear how this has happened. Synchronically, speakers no longer parse the two components as separate items.\footnote{Hence, it might also be written as one word, \emph{näbun} instead of \emph{nä bun}.} This is reflected in its grammatical behaviour: \emph{nä bun} can be marked for the same range of cases as personal pronouns, and like personal pronouns it may consitute a complete \isi{noun phrase}. \tabref{indefpron-table} below lists all the \isi{case} forms of \emph{nä bun}.%\\
...
Following Diessel (\citeyear{Diessel:2009tg}), I will outline the syntactic distribution of demonstratives first. In \tabref{demonstratives-table}, a number of demonstratives appear in shaded cells. These have additional functions and to some extent different syntactic distributions. They will be discussed in separate sections to follow.%\\
...
Komnzo possesses a set of \isi{deictic} verbal proclitics which I call \isi{clitic} demonstratives. Compare \tabref{demonstratives-table} above. They are used for identification and can attach to any inflected verb. In example (\ref{ex067}) two brothers are trying to kill a creature by shooting an arrow into its heart.
...
Komnzo ideophones depict almost exlusively sounds and, thus, cover the lower spectrum of the implicational hierarchy of sensory imagery as discussed in \citep[663]{Dingemanse:2012fc}. Komnzo ideophones cover a range of audititory phenomena: sounds from nature, animal sounds, human made noises, bodily noises, human made signals. \tabref{kwan-words} groups them according to their semantics.%\\
